...
class TestGStreamerWebRTCManager:
    ...
    def test_get_launch_string(self, gstreamer_webrtc_manager):
        ...
        assert result2 == ' appsrc name=webrtc_source format=GST_FORMAT_TIME  caps="video/x-raw,width=1920,height=1080"  ! videoconvert ! gvawatermark displ-cfg=font-scale=1.0,draw-txt-bg=false ! x264enc speed-preset=ultrafast name=h264enc ! video/x-h264,profile=baseline  ! whipclientsink signaller::whip-endpoint= http://10.10.10.10:8889/peer1/whip'
        ...
        assert result_jpeg == ' appsrc name=webrtc_source format=GST_FORMAT_TIME  caps="image/jpeg,width=1920,height=1080"  ! jpegdec ! videoconvert ! gvawatermark displ-cfg=font-scale=1.0,draw-txt-bg=false ! x264enc speed-preset=ultrafast name=h264enc  ! video/x-h264,profile=baseline  ! whipclientsink signaller::whip-endpoint= http://10.10.10.10:8889/peer1/whip'
